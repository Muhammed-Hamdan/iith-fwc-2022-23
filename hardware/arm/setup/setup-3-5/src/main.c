...
int main(void)
{
    ...
    uint8_t on_color = PINK, off_color = OFF;
    ...
    //LED pins Output
    PyHal_GPIO_SetDir(RED_LED, GPIO_OUTPUT_MODE);
    PyHal_GPIO_SetDir(GREEN_LED, GPIO_OUTPUT_MODE);
    PyHal_GPIO_SetDir(BLUE_LED, GPIO_OUTPUT_MODE);
    PyHal_GPIO_SetDir(INPUT, GPIO_INPUT_MODE);

    while(1){
	    
	    if(!PyHal_GPIO_Get(INPUT)){
		    on_color = PINK; 
		    off_color = OFF;
	    }
	    else{
            ...
            PyHal_GPIO_Set(RED_LED, on_color&(1<<2));
            PyHal_GPIO_Set(GREEN_LED, on_color&(1<<1));
            PyHal_GPIO_Set(BLUE_LED, on_color&(1<<0));
            HAL_DelayUSec(1000000);    

            PyHal_GPIO_Set(RED_LED, off_color&(1<<2));
            PyHal_GPIO_Set(GREEN_LED, off_color&(1<<1));
            PyHal_GPIO_Set(BLUE_LED, off_color&(1<<0));
            HAL_DelayUSec(1000000);    
