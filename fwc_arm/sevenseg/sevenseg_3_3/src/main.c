...
void gpio_init(){
	PyHal_GPIO_SetDir(A_PIN, GPIO_OUTPUT_MODE);
	PyHal_GPIO_SetDir(B_PIN, GPIO_OUTPUT_MODE);
	PyHal_GPIO_SetDir(C_PIN, GPIO_OUTPUT_MODE);
	PyHal_GPIO_SetDir(D_PIN, GPIO_OUTPUT_MODE);
	PyHal_GPIO_SetDir(E_PIN, GPIO_OUTPUT_MODE);
	PyHal_GPIO_SetDir(F_PIN, GPIO_OUTPUT_MODE);
	PyHal_GPIO_SetDir(G_PIN, GPIO_OUTPUT_MODE);
}

void sevenseg(uint8_t num){
	uint8_t code;
	code = (num <= 9) ? ~sevseg_code_arr[num] : ~0x00;
	PyHal_GPIO_Set(A_PIN, code&(1<<0));
	PyHal_GPIO_Set(B_PIN, code&(1<<1));
	PyHal_GPIO_Set(C_PIN, code&(1<<2));
	PyHal_GPIO_Set(D_PIN, code&(1<<3));
	PyHal_GPIO_Set(E_PIN, code&(1<<4));
	PyHal_GPIO_Set(F_PIN, code&(1<<5));
	PyHal_GPIO_Set(G_PIN, code&(1<<6));
}
